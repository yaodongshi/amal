...
class AccountPartialReconcile(models.Model):
    ...
    def create_exchange_rate_entry(self, aml_to_fix, move):
        print('exech', aml_to_fix, move)
        created_lines, partial_rec = super(AccountPartialReconcile, self).create_exchange_rate_entry(aml_to_fix, move)
        print('exech', created_lines, partial_rec)
        for each in move.line_ids:
            each.branch_id = aml_to_fix[0].branch_id
            each.branch_id = aml_to_fix[0].branch_id
        move.branch_id = aml_to_fix[0].branch_id
...
class AccountInvoice(models.Model):
    ...
    def action_invoice_open(self):
        ...
        for each in self:
            if each.branch_id:
                for each_move in each.move_id:
                    each_move.branch_id = each.branch_id
                    for each_move_line in each_move.line_ids:
                        each_move_line.branch_id = each.branch_id
    ...
    #         print(total, total_currency,'ddddddddddddd')
    #         if inv.use_custom_rate:
    #             total=total * inv.custom_rate
    #             total_currency=total_currency * inv.custom_rate
    #         print(total, total_currency,'ddddddddddddd')
    #
    #         name = inv.name or ''
        ...
        if i_line.product_id and i_line.product_id.valuation == 'real_time' and i_line.product_id.type == 'product':
            ...
            for line in res:
                if line.get('invl_id', 0) == i_line.id and reference_account_id == line['account_id']:
                    ...
                    if float_compare(valuation_price_unit, i_line.price_unit,
...
class AccountMoveLine(models.Model):
    ...
    branch_id = fields.Many2one('res.branch', related='move_id.branch_id',default=_default_branch_id,store=True)
